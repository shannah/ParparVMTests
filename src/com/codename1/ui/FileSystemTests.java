...
public class FileSystemTests extends AbstractTest {
    ...
    private void testFileClass() throws Exception {
        ...
        if (BrowserComponent.isNativeBrowserSupported()) {
			...
			while (!res.complete && System.currentTimeMillis() - startTime < timeout) {
				Display.getInstance().invokeAndBlock(()->{
					Util.sleep(100);
				});
			}
                        if (!res.complete) {
                            res.error = new RuntimeException("Timeout reached trying to load webpage from local file: "+hello.toURL().toString());
                        }
			if (res.error != null) {
